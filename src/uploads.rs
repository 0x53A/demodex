...
use std::{io::Write, os::unix::fs::{DirBuilderExt, OpenOptionsExt}, path::Path};

pub const MAX_BYTES: usize = 4 * 1024 * 1024;

pub fn extension(bytes: &[u8]) -> Result<&'static str> {
    ensure!(!bytes.is_empty() && bytes.len() <= MAX_BYTES, "Images must be between 1 byte and 4 MiB");
...
mod tests {
    ...
    fn validates_size_and_format_and_preserves_private_unique_files() -> Result<()> {
        ...
        assert_eq!(std::fs::metadata(first)?.permissions().mode() & 0o777, 0o600);
        Ok(())
    }
}
