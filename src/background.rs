...
pub(crate) async fn list(live: &Live) -> Result<Vec<Value>> {
    ...
    for _ in 0..100 {
        ...
        for row in page["data"]
            .as_array()
            .context("Invalid background terminal list")?
        {
            for field in ["processId", "itemId", "command", "cwd"] {
                ensure!(
                    row[field].is_string(),
                    "Invalid background terminal {field}"
                );
            }
            if !rows
                .iter()
                .any(|old: &Value| old["processId"] == row["processId"])
            {
                rows.push(row.clone());
            }
        }
        cursor = page
            .get("nextCursor")
            .context("Missing background terminal cursor")?
            .clone();
        if cursor.is_null() {
            return Ok(rows);
        }
        let next = cursor
            .as_str()
            .context("Invalid background terminal cursor")?;
        ensure!(
            cursors.insert(next.to_owned()),
            "Repeated background terminal cursor"
        );
    }
...
impl Manager {
    pub async fn background_snapshot(&self, id: &str) -> Value {
        let result = tokio::time::timeout(std::time::Duration::from_secs(5), async {
            ...
            let rows = list(&live).await?;
                ...
                "Codex connection changed; refresh background terminals"
            );
    ...
    pub async fn stop_background(
        ...
        processes: &[(String, String)],
            ...
            !processes.is_empty() && processes.len() <= 10000,
            "Select background terminals to stop"
        );
        let _guard = self.connecting.lock().await;
        let live = self.runtime(id).await?;
        ensure!(
            live.generation == generation,
            "These process handles belong to an old Codex connection. Refresh before stopping terminals."
        );
        let rows = list(&live).await?;
        ...
        for (process, item) in processes {
            if let Some(row) = rows.iter().find(|row| row["processId"] == *process) {
                    ...
                    row["itemId"] == *item,
                    "Background process identity changed; refresh before stopping it"
                );
            }
        }
        ...
        for (process, _) in processes {
            ...
            let response = response?;
                ...
                response["terminated"].is_boolean(),
                "Invalid terminal termination response"
            );
