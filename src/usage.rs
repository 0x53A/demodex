...
use std::{sync::{Arc, Weak}, time::{Duration, Instant, SystemTime, UNIX_EPOCH}};
...
pub fn weekly(response: &Value) -> Value {
    ...
    let mut add = |id: &str, bucket: &Value| {
        for name in ["primary", "secondary"] {
            ...
            if window["windowDurationMins"].as_i64() != Some(7 * 24 * 60) { continue; }
            let Some(used) = window["usedPercent"].as_i64().filter(|v| (0..=100).contains(v)) else { continue; };
            windows.push(json!({"id":id,"name":bucket["limitName"].as_str().unwrap_or(id),
                "used_percent":used,"resets_at":window["resetsAt"]}));
        }
    };
    if let Some(buckets) = response["rateLimitsByLimitId"].as_object().filter(|m|!m.is_empty()) {
        if let Some(bucket) = buckets.get("codex") { add("codex", bucket); }
        for (id, bucket) in buckets { if id != "codex" { add(id, bucket); } }
    } else {
        add(response["rateLimits"]["limitId"].as_str().unwrap_or("codex"), &response["rateLimits"]);
    }
    json!({"windows":windows,"checked_at":now(),"error":if windows.is_empty(){Some("No weekly usage window reported by Codex")}else{None}})
}

struct Cached { rpc: Weak<Rpc>, account: Value, checked: Instant, value: Value }
#[derive(Default)]
...
impl RateLimitsCache {
    pub async fn read(&self, rpc: Arc<Rpc>, account: &Value) -> Value {
        ...
        if let Some(entry) = cache.as_ref()
            && entry.account == *account && entry.checked.elapsed() < Duration::from_secs(60)
            && entry.rpc.upgrade().is_some_and(|old|Arc::ptr_eq(&old,&rpc)) {
            return entry.value.clone();
        }
        let value = match tokio::time::timeout(Duration::from_secs(5), rpc.call("account/rateLimits/read", json!({}))).await {
            Ok(Ok(response)) => weekly(&response),
            Ok(Err(error)) => json!({"windows":[],"checked_at":now(),"error":format!("Weekly usage unavailable: {error:#}")}),
            Err(_) => json!({"windows":[],"checked_at":now(),"error":"Weekly usage request timed out"}),
        };
        *cache = Some(Cached {rpc:Arc::downgrade(&rpc),account:account.clone(),checked:Instant::now(),value:value.clone()});
        value
    }
}
...
mod tests {
    use super::*;
    #[tokio::test]
    async fn account_cache_deduplicates_reads_and_does_not_leak_across_accounts() -> anyhow::Result<()> {
            ...
            while let Some(Ok(Message::Text(text)))=socket.next().await {
                ...
                let reply=match request["method"].as_str().unwrap() {
                    ...
                    "account/rateLimits/read"=>{
                        ...
                        if count==2 {json!({"id":request["id"],"error":{"message":"unavailable"}})}
                        else {json!({"id":request["id"],"result":{"rateLimits":{"secondary":{"usedPercent":20,"windowDurationMins":10080}}}})}
                    },
        ...
        let (a,b)=tokio::join!(cache.read(rpc.clone(),&first),cache.read(rpc.clone(),&first));
        assert_eq!(a,b);
    ...
    fn context_history_migrates_and_latest_snapshot_survives_restart() -> anyhow::Result<()> {
        ...
        let id;
        {
            ...
            id=store.create("test","ws://localhost:1",&[],Some("thread"))?.id;
        ...
        }
        {
            ...
            store.context_usage(&id,&json!({"last":{"totalTokens":0},"modelContextWindow":200000}))?;
        }
        let store=crate::store::Store::open(&path)?;
    ...
    fn weekly_selects_duration_not_primary_position_and_avoids_duplicate_legacy_bucket() {
        ...
        assert!(weekly(&json!({"rateLimits":{"secondary":{"usedPercent":0}}}))["windows"].as_array().unwrap().is_empty());
        assert_eq!(weekly(&json!({"rateLimits":{"secondary":{"usedPercent":0,"windowDurationMins":10080}}}))["windows"][0]["used_percent"],0);
    }
    #[test]
    fn context_uses_last_not_cumulative_tokens_and_can_decrease_after_compaction() {
        let first=context(&json!({"last":{"totalTokens":60000},"total":{"totalTokens":900000},"modelContextWindow":200000}));
        assert_eq!(first["used_tokens"],60000);
        assert_eq!(context(&json!({"last":{"totalTokens":10000},"modelContextWindow":200000}))["used_tokens"],10000);
