...
impl Manager {
    ...
    pub async fn archive(&self, id: &str, archived: bool) -> Result<()> {
        ...
        if archived {
            anyhow::ensure!(matches!(session.status.as_str(), "idle" | "connected" | "disconnected"),
                "Stop the session before archiving it");
            anyhow::ensure!(!self.store.pending(id)?.iter().any(|p|matches!(p.state.as_str(), "pending" | "responding" | "delivered")),
                "Resolve pending decisions before archiving the session");
            if let Ok(live) = self.runtime(id).await {
                Self::require_idle(&live).await?;
                anyhow::ensure!(crate::background::list(&live).await?.is_empty(), "Stop background terminals before archiving the session");
                anyhow::ensure!(self.queued(id).await?.as_array().is_some_and(|q|q.is_empty()),
                ...
                anyhow::ensure!(goal.get("goal").is_some(), "Cannot verify the session goal state");
                anyhow::ensure!(goal["goal"]["status"] != "active", "Pause the goal before archiving the session");
                let turn = live.turn.lock().await;
                anyhow::ensure!(turn.is_none(), "Stop the session before archiving it");
                self.store.archive(id, true)?;
            } else {
                anyhow::ensure!(session.status == "disconnected", "Cannot verify that the session is stopped");
                self.store.archive(id, true)?;
            }
        } else {
    ...
    pub async fn connect(self: &Arc<Self>, id: &str) -> Result<()> {
        ...
        anyhow::ensure!(!self.store.get(id)?.archived, "Restore the session before resuming it");
    ...
    async fn ingest(&self, id: &str, live: &Live, message: &Value) -> Result<()> {
        ...
        let mut turn = if matches!(method, "turn/started" | "turn/completed" | "thread/status/changed") {
        ...
        if !sessions.get(id).is_some_and(|s| s.generation == live.generation) {
            return Ok(());
        }
        ...
        if method == "item/tool/call" && message.get("id").is_some() {
            ...
            let result = self.store.context_tool(id, &live.thread, &message["params"])
                .unwrap_or_else(|error| crate::session_context::response(Err(error)));
            drop(sessions);
            self.changed();
            live.rpc.send(json!({"id":message["id"],"result":result})).await?;
            return Ok(());
        }
            ...
            match method {
                ...
                "thread/settings/updated" => {
                    ...
                    if settings["model"].is_string() {
                        ...
                        let _ = self.model_updates.send((id.into(),live.generation.clone(),accepted));
                    }
                    let policy = &message["params"]["threadSettings"]["sandboxPolicy"];
                    if policy["type"].is_string() {
                        self.store.effective_sandbox(id, policy)?;
                        let _ = self.settings_updates.send((id.into(), live.generation.clone(), policy.clone()));
                    }
                }
                _ => {}
            }
    ...
    pub async fn prompt(&self, id: &str, text: &str) -> Result<Value> {
        let _settings = self.connecting.lock().await;
        anyhow::ensure!(!self.store.get(id)?.archived, "Restore the session before sending a message");
        ...
        let steered = if let Some(active_turn) = turn.as_ref() {
            match live.rpc.call("turn/steer", json!({"threadId":live.thread,
                "expectedTurnId":active_turn,"input":[{"type":"text","text":text}],
                "clientUserMessageId":client_message_id})).await {
                ...
                Err(error) if crate::rpc::no_active_turn(&error) => {
                    ...
                    let state = live.rpc.call("thread/read",json!({"threadId":live.thread,"includeTurns":false})).await?;
                    if state["thread"]["status"]["type"] != "idle" { return Err(error); }
                    None
                },
        ...
        let (result, method) = if let Some(result) = steered {
            ...
            let result = live.rpc.call("turn/start",json!({"threadId":live.thread,
                "clientUserMessageId":client_message_id,"input":[{"type":"text","text":text}],
                "environments":environment_params(&session.targets)})).await?;
    ...
    pub async fn queue_prompt(&self, id: &str, text: &str) -> Result<Value> {
        let _settings = self.connecting.lock().await;
        anyhow::ensure!(!self.store.get(id)?.archived, "Restore the session before queueing a message");
        anyhow::ensure!(!text.trim().is_empty(), "Message must not be empty");
        anyhow::ensure!(!self.store.targets_pending(id)?, "Send a message to apply selected targets before queueing work");
        let live = self.runtime(id).await?;
        let turn = live.turn.lock().await;
        anyhow::ensure!(turn.is_some(), "The session is idle. Use Send to start work; nothing was queued");
    ...
    pub async fn queued(&self, id: &str) -> Result<Value> {
        ...
        loop {
            let page = live.rpc.call("thread/queue/list", json!({"threadId":live.thread,"limit":100,"cursor":cursor})).await?;
            data.extend(page["data"].as_array().context("Codex did not return a message queue")?.iter().cloned());
            let next = page.get("nextCursor").context("Missing message queue cursor")?.clone();
            if next.is_null() { break; }
            let key = next.as_str().context("Invalid message queue cursor")?;
            anyhow::ensure!(cursors.len() < 100 && cursors.insert(key.to_owned()) && data.len() < 10000, "Invalid message queue pagination");
            cursor = next;
        }
        Ok(json!(data))
    }
    ...
    pub async fn cancel_queued(&self, id: &str, queued_id: &str) -> Result<Value> {
        let live = self.runtime(id).await?;
        let result = live.rpc.call("thread/queue/delete", json!({"threadId":live.thread,"queuedSubmissionId":queued_id})).await?;
        self.changed();
        Ok(result)
    }
    ...
    pub async fn resume_queue(&self, id: &str) -> Result<Value> {
        let _settings = self.connecting.lock().await;
        anyhow::ensure!(!self.store.get(id)?.archived, "Restore the session before resuming queued work");
        anyhow::ensure!(!self.store.targets_pending(id)?, "Send a message to apply the selected targets before resuming the queue");
        let live = self.runtime(id).await?;
        let mut turn = live.turn.lock().await;
        anyhow::ensure!(turn.is_none(), "Wait for the active turn before resuming the queue");
        let result = live.rpc.call("thread/queue/start", json!({"threadId":live.thread})).await?;
        *turn = result["turn"]["id"].as_str().map(str::to_owned);
        self.changed();
        Ok(result)
    }
    ...
    pub async fn change_sandbox(&self, id: &str, sandbox: Option<crate::store::Sandbox>) -> Result<()> {
        ...
        if let Ok(live) = self.runtime(id).await {
            ...
            if sandbox.is_some() {
                ...
                if let Err(error) = live.rpc.call("thread/settings/update", json!({"threadId":live.thread,"sandboxPolicy":policy})).await {
                ...
                self.changed();
                anyhow::ensure!(accepted["type"] == policy["type"],
                    "Codex reported sandbox {} instead of requested {}", accepted["type"], policy["type"]);
                return Ok(());
            }
...
mod tests {
    ...
    async fn old_completion_cannot_clear_the_fallback_turn() -> Result<()> {
        ...
        let live=Arc::new(Live{rpc,generation:"generation".into(),thread:"thread".into(),turn:Mutex::new(Some("new".into()))});
    ...
    async fn queue_pagination_rejects_cycles_even_when_pages_are_empty() -> Result<()> {
        ...
        let fake = tokio::spawn(async move {
            ...
            while let Some(Ok(Message::Text(raw))) = ws.next().await {
                ...
                let result = match request["method"].as_str().unwrap() {
                    ...
                    "initialize" => json!({}),
                    "thread/queue/list" => json!({"data":[],"nextCursor":if request["params"]["cursor"] == "a" {"b"} else {"a"}}),
        ...
        let error = tokio::time::timeout(std::time::Duration::from_secs(2), manager.queued(&session.id)).await?.unwrap_err();
    ...
    async fn active_turn_cannot_change_sandbox() -> Result<()> {
        ...
        manager.store.sandbox(&session.id, Some(crate::store::Sandbox::DangerFullAccess))?;
        ...
        assert_eq!(manager.store.get(&session.id)?.sandbox, Some(crate::store::Sandbox::DangerFullAccess));
        manager.disconnect(&session.id, "done").await?;
        fake.await?;
        Ok(())
    }
    async fn sandbox_confirmation_fixture(accepted: Option<Value>, response: &'static str) -> Result<()> {
        use futures_util::{SinkExt, StreamExt};
        use tokio_tungstenite::tungstenite::Message;
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await?;
        let url = format!("ws://{}", listener.local_addr()?);
        ...
        let fake = tokio::spawn(async move {
            ...
            while let Some(Ok(Message::Text(text))) = ws.next().await {
                ...
                let result = match method {
                    "initialize" => json!({}),
                    "thread/resume" => json!({"thread":{"id":"thread","turns":[]},"sandbox":{"type":"readOnly","networkAccess":false}}),
                    ...
                    "thread/settings/update" => {
                        ...
                        if response == "reject" {
                            ws.send(Message::Text(json!({"id":request["id"],"error":{"message":"fixture rejection"}}).to_string().into())).await.unwrap();
                            continue;
                        }
        ...
        let manager = Manager::new(Store::open(std::path::Path::new(":memory:"))?);
        let session = manager.store.create("settings", &url, &[], Some("thread"))?;
        manager.store.sandbox(&session.id, Some(crate::store::Sandbox::DangerFullAccess))?;
        ...
        if response == "disconnect" {
            ...
            assert!(result.unwrap_err().to_string().contains("fixture rejection"));
    ...
    async fn reports_codex_policy_instead_of_requested_policy() -> Result<()> {
        sandbox_confirmation_fixture(Some(json!({"type":"readOnly","networkAccess":false})), "accept").await
    }
    #[tokio::test]
