...
pub struct Modal { dialog: NodeRef }
impl Component for Modal {
    type Message = ();
    type Properties = Props;
    fn create(_: &Context<Self>) -> Self { Self { dialog: NodeRef::default() } }
    ...
    fn update(&mut self, ctx: &Context<Self>, _: ()) -> bool {
        if let Some(dialog) = self.dialog.cast::<HtmlDialogElement>() { dialog.close(); }
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let trap_focus = Callback::from(move |event: KeyboardEvent| {
            ...
            let Some(dialog) = reference.cast::<HtmlDialogElement>() else { return; };
            let Ok(nodes) = dialog.query_selector_all("button,input,select,textarea,a[href],summary,[tabindex]") else { return; };
            let focusable: Vec<HtmlElement> = (0..nodes.length()).filter_map(|index| nodes.item(index)?.dyn_into::<HtmlElement>().ok())
                .filter(|element| !element.matches(":disabled").unwrap_or(true) && element.tab_index() >= 0 && (element.offset_width() > 0 || element.offset_height() > 0)).collect();
            let Some(active) = web_sys::window().and_then(|w|w.document()).and_then(|d|d.active_element()) else { return; };
            if let (Some(first), Some(last)) = (focusable.first(), focusable.last()) {
                let target = if event.shift_key() && first.unchecked_ref::<web_sys::Element>() == &active { Some(last) }
                    else if !event.shift_key() && last.unchecked_ref::<web_sys::Element>() == &active { Some(first) } else { None };
