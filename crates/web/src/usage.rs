...
pub fn context(session: &Value, detailed: bool) -> Html {
    match context_numbers(session) {
        Some((used, window, percent)) => {
            ...
            let title = format!("Last reported context: {used} / {window} tokens. Updated when Codex reports usage; not cumulative session tokens.");
        ...
        None => html!{<span class="context-usage unavailable" title="Waiting for Codex to report token usage and the model context window">{"Context unavailable"}</span>},
    }
}

fn timestamp(seconds: &Value) -> String {
    seconds.as_f64().filter(|n|n.is_finite() && *n>0.0).map(|n| {
        js_sys::Date::new(&wasm_bindgen::JsValue::from_f64(n*1000.0))
            .to_locale_string("default", &wasm_bindgen::JsValue::UNDEFINED).as_string().unwrap_or_default()
...
pub fn weekly(runtime: &Value, connected: bool) -> Html {
    ...
    let summary = if !connected { "Weekly usage · disconnected".into() }
        else if let Some(first) = windows.first() {
            let suffix=if windows.len()>1{format!(" · {} limits",windows.len())}else{String::new()};
            format!("Weekly usage · {}% used{}",first["used_percent"].as_i64().unwrap_or(0),suffix)
        } else { "Weekly usage unavailable".into() };
...
mod tests {
    ...
    fn context_percentage_uses_reported_window_without_assuming_model_capacity() {
        assert_eq!(context_numbers(&json!({"context_usage":{"used_tokens":50000,"window_tokens":200000}})),Some((50000,200000,25.0)));
        assert!(context_numbers(&json!({"context_usage":{"used_tokens":50000,"window_tokens":null}})).is_none());
        assert!(context_numbers(&json!({})).is_none());
        assert_eq!(context_numbers(&json!({"context_usage":{"used_tokens":0,"window_tokens":100}})).unwrap().2,0.0);
    }
}
