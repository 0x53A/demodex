...
impl App {
    pub(super) fn new_session_targets(&self) -> Vec<Value> {
        self.saved.fields.get("new_targets")
            .and_then(|value| serde_json::from_str(value).ok()).unwrap_or_default()
    }

    pub(super) fn new_session_view(&self, ctx: &Context<Self>) -> Html {
        if !self.show_new_session { return Html::default(); }
        let chosen = self.new_session_targets();
        let ssh = chosen.iter().any(|target| text(target,"id").starts_with("ssh-"));
        let ready = !chosen.is_empty() && chosen.iter().all(|target| {
            text(target,"cwd").starts_with('/') && self.targets.iter().any(|known| known["id"]==target["id"] && known["available"]==true)
        });
